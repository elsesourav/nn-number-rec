...
class NeuralNetwork {
   ...
   train(inputArray, targetArray) {
      ...
      this.$outputDeltas = Matrix.multiply(
         this.$outputErrors,
         this.$outputDerivs
      );
      ...
      // calculate the hidden1 deltas (errors * derivative of hidden1)
      this.$hidden1Derivs = Matrix.map(this.hidden1, (x) => sigmoid(x, true));
      this.$hidden1Deltas = Matrix.multiply(
         this.$hidden1Errors,
         this.$hidden1Derivs
      );
      ...
      this.$hidden0Deltas = Matrix.multiply(
         this.$hidden0Errors,
         this.$hidden0Derivs
      );
      ...
      this.weights2.add(
         Matrix.map(
            Matrix.dot(this.$hidden1T, this.$outputDeltas),
            (x) => x * this.lrnRate
         )
      );
      ...
      this.weights1.add(
         Matrix.map(
            Matrix.dot(this.$hidden0T, this.$hidden1Deltas),
            (x) => x * this.lrnRate
         )
      );
      ...
      this.weights0.add(
         Matrix.map(
            Matrix.dot(this.$inputsT, this.$hidden0Deltas),
            (x) => x * this.lrnRate
         )
      );
